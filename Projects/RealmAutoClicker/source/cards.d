...
class CardManager
{
	...
	Card instantiateCard(Card,Args...)(Args args)
	{
		auto windowSize = RealmApp.getWindowSize();
		Card card = instantiate!(Card)(args);
		
		Transform cardTransform = card.getComponent!(Transform)();
		cardTransform.position = vec3((windowSize[0] /-2) + (210 * numCards) + 100,cardTransform.position.y,cardTransform.position.z);
		numCards++;
		return card;
...
mixin template Card(alias string CardName,int LuckyNumber,int Reward)
{
	...
	void start(ref IFImage image,vec3 position)
	{
		...
		transform.scale = vec3(200,300,1);
		...
		cardInfo = RealmUI.createElement(vec3(0,-112.5,0),vec3(200,75,1),vec3(0,0,0));
